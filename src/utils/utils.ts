...
export const convertCountryA3ToA2 = (code = '') => {
    ...
    const out = countries[code]
    return out
}

export const getBoundsByCountry = (
    alpha3Code: string
): [[number, number], [number, number]] => {
    const featureCollection = getFeatureCollectionByCountry(alpha3Code)
    const bb = bbox(featureCollection)
    return [
        [bb[0], bb[1]],
        [bb[2], bb[3]]
    ]
...
export const getFeatureCollectionByCountry = (
    alpha3Code: string
): FeatureCollection => {
    return {
        type: 'FeatureCollection',
        features: boundaryData.features.filter(
            (feature) => feature.properties.ISO_A3 === alpha3Code
        )
    }
}
...
export const getFitBounds = (
    ...
    const bounds = getBoundsByCountry(alpha3Code)
